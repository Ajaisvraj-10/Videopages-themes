...
Vue.prototype.$appName = 'Ott';
Vue.prototype.api_url = 'http://api.ott.capcee.com';
...
Vue.prototype.advance_types = ['Category','Meta Tags','Language']
...
Vue.mixin({
  data() {
    return {
      ...
      load_setting:false,
      libary_id:null,
      frontent_settings:{main_logo:'',name:'',have_plan:false},
      social_links:[],
    };
  },
  mounted() {
  ...
  methods: {
    ...
    paginationMovies(items,page=10){
      var datas=[];
      ...
      if(items.length>page){
        if(datas[key-1].length<actual_page){
          var balance = actual_page-datas[key-1].length
          items.forEach(function(value,mkey){
            if(mkey<balance){
              datas[key-1].push(value)
            }
          })
        }
      }
