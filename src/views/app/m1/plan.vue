      ...
      <div class="px-3 py-2 w-100 plan-header" >
        <div class="flex-align">
            ...
            <div class="text-center header-right flex-align3" style="gap:20px;">
                      <div v-if="$root.token">
                        <button @click="signOut()" class="btn rounded2 btn-light foucs-btn" style="font-size: 13px;font-weight: 700;">LOGOUT</button>
                      </div>
                      <router-link :to="edit?'':{name:'sign-in'}" v-else>
                          <button class="btn rounded2 btn-light foucs-btn" style="font-size: 13px;font-weight: 700;">LOGIN</button>
                      </router-link>
             </div>
          </div>
      </div>
      <div class="plan-container">
        <div class="plan-sec px-3 " v-if="crnt_tab=='plans'">
          ...
          <div class="plan-card row mx-0">
            ...
            <div class="col-md-7 plan-card-col" style="display: flex;flex-direction: column;justify-content: center;">
              <div class="mb-3">
              ...
              <button class="btn btn-light w-100 rounded2 py-2" v-if="current_plan.id" @click="getPaymentMethods();crnt_tab='payment_methods'">Continue with {{current_plan.currency_symbol}}{{current_plan.price}}</button>
            </div>

            </div>
            <div class="col-md-5 plan-card-col plan-prize-card pt-3" style="display: flex;width: 100%;overflow: auto;flex-direction: column;align-items: center;">
              <button class="plans-card mb-3 foucs-btn" v-for="plan in plans" :style="plan.id==current_plan.id?'background:#0f171e; border-radius:6px;':'border-radius:6px;'" @click="current_plan=plan">
                <div class="w-100 p-3 rounded2-top position-relative" :style="plan.id==current_plan.id?'background: #23384a; border-radius:6px 6px 0 0;':'border-radius:6px 6px 0 0;padding-bottom:0!important;'" @click="current_plan=plan">
                  ...
                  <div class="d-flex align-items-center justify-content-between ">
                    <div >
                      <div class="text-white font-weight-600  text-14">
                ...
                <div class="w-100 rounded2-bottom px-3 py-2 text-center text-white  text-14 font-weight-500 flex-align" >
                  <div class="text-12 text-left">
                    {{plan.description}}
                  </div>
                  <div class="text-12 mt-2">
                         <!-- <sup class="text-white ">₹999</sup> --><span class="text-white text-16 font-weight-600 ">{{plan.currency_symbol}}{{plan.price}}</span>
                  </div>
                </div>
        ...
        <div class="plan-sec2 px-3" v-if="crnt_tab=='payment_methods'">
          ...
          <div class="plan-card2 row mx-0">
            <div class="col-md-12 plan-card-col">
              ...
              <button class="btn btn-light rounded2 py-2 w-100 my-2" v-for="payment_method in payment_methods" @click="setRazorpay(payment_method);crnt_tab='pay'">{{payment_method.name}}</button>
              ...
              <div class="d-flex justify-content-center plans-social-icons my-3" style="gap:20px;">
                <img src="/all-images/apple.svg" style="width:40px;">
                <img src="/all-images/google.svg" style="width:40px;">
                <img src="/all-images/fb.svg" style="width:40px;">
                <img src="/all-images/twitter.svg" style="width:40px;">
              </div>
        ...
        <div class="plan-sec2 px-3 mt-5" v-if="crnt_tab=='pay'">
          ...
          <div class="plan-card2 row mx-0">
            <div class="col-md-12 plan-card-col">
              ...
              <h3 class="text-white my-3 font-weight-500 text-center">{{current_plan.currency_symbol}}{{current_plan.price}}</h3>
              <button class="btn btn-light rounded2 py-2 w-100 my-2" @click="purchasePlan()" v-if="razorpay_options.key">Pay Now</button>
              <h6 class="text-center text-12 mt-2 cursor-pointer" @click="crnt_tab='payment_methods'">Change method</h6>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="site-wrapper loading-plan" v-else> 
      <div class="px-3 py-2 w-100 plan-header animated-background" style="background: linear-gradient(to right, #0a060d 8%, #160e1d 18%, #1a1120 33%);">
      </div>
      <div class="plan-container">
        <div class="plan-sec px-3 ">
          <div class="plan-card row mx-0">
            <div class="col-md-7 plan-card-col" style="display: flex;flex-direction: column;justify-content: center;">
              <div class="mb-2">
                <h6 class="mx-auto height2 animated-background px-5 mb-3" style="width:150px;"></h6>
                <h6 class="height2 width9 animated-background px-5 mb-2"></h6>
                <h6 class="height2 width9 animated-background px-5 mb-2" style="width:150px;"></h6>
              </div>
              <h6 class="height3 width9 animated-background px-5 mb-2" style="width:250px;"></h6>
              <div class="mb-1">
                <h6 class="height5 width9 animated-background px-5 mb-2" style="width:100%;"></h6>
              </div>
              <div class="mb-1">
                <h6 class="height5 width9 animated-background px-5 mb-2" style="width:100%;"></h6>
              </div>
              ...
              <h6 class="height6 animated-background px-5 mt-2" style="width:100%;"></h6>
            </div>
            <div class="col-md-5 plan-card-col plan-prize-card pt-3" style="display: flex;width: 100%;overflow: auto;flex-direction: column;align-items: center;">
              <div class=" mb-3 animated-background" style="height:100px;width:200px;"></div>
              <div class=" mb-3 animated-background" style="height:80px;width:200px;"></div>
              <div class=" mb-3 animated-background" style="height:80px;width:200px;"></div>
              <div class=" mb-3 animated-background" style="height:80px;width:200px;"></div>
            </div>
          </div>
...
.plan-header {
    background-image:linear-gradient(rgb(0 0 0 / 12%), #0f171e), url(/all-images/bg1.jpg);
    min-height: 150px;
