...
<template>
  <div class="main-content capcee-plan">
      ...
      <div class="px-3 py-2 w-100 plan-header" >
        <div class="flex-align">
            ...
            <div class="text-center header-right flex-align3" style="gap:20px;">
                      <div v-if="$root.token">
                        <button @click="signOut()" class="btn rounded2" style="background:linear-gradient(45deg, #8230c6, #7d208e);color: white;font-size: 13px;font-weight: 500;border: 1px solid #8230c6;border-radius: 5px;">LOGOUT</button>
                      </div>
                      <router-link :to="edit?'':{name:'sign-in'}" v-else>
                          <button class="btn rounded2" style="background:linear-gradient(45deg, #8230c6, #7d208e);color: white;font-size: 13px;font-weight: 500;border: 1px solid #8230c6;border-radius: 5px;">LOGIN</button>
                      </router-link>
             </div>
          </div>
      </div>
        ...
        <div class="plan-sec px-3 " v-if="crnt_tab=='plans'">
          ...
          <div class="plan-card row mx-0">
            ...
            <div class="col-md-7 plan-card-col" style="display: flex;flex-direction: column;justify-content: center;">
              <div class="mb-3">
                <h6 class="text-dark text-14 text-center mb-3 pb-3">Choose your premium plan</h6>
            ...
            <div class="col-md-5 plan-card-col plan-prize-card pt-3" style="display: flex;width: 100%;overflow: auto;flex-direction: column;align-items: center;">
              <div class="plans-card mb-3" v-for="plan in plans" :style="plan.id==current_plan.id?'background: #673ab714;color:white; border-radius:6px;':'border-radius:6px;'" @click="current_plan=plan">
                <div class="p-3 rounded2-top position-relative" :style="plan.id==current_plan.id?'background: #7e249c;color:white; border-radius:4px 4px 0 0;':'border-radius:6px 6px 0 0;padding-bottom:0!important;'" @click="current_plan=plan">
                  ...
                  <div class="d-flex align-items-center justify-content-between ">
                    ...
                    <div class="d-flex" >
                      ...
                      <template v-if="plan.id==current_plan.id">
                        <i class="fa fa-check ml-2 bg-success flex-align2 rounded-circle text-dark" style=""></i>
                      </template>
                      <template v-else>
                ...
                <div class="rounded2-bottom px-3 py-2 text-center text-dark  text-14 font-weight-500 flex-align" >
                  <div class="text-12 text-left">
                    {{plan.description}}
                  </div>
                  <div class="text-12 mt-2">
                         <!-- <sup class="text-dark ">₹999</sup> --><span class="text-dark text-16 font-weight-600 ">{{plan.currency_symbol}}{{plan.price}}</span>
                  </div>
                </div>
        ...
        <div class="plan-sec2 px-3 mt-5" v-if="crnt_tab=='pay'">
          ...
          <div class="plan-card2 row mx-0">
            <div class="col-md-12 plan-card-col">
              ...
              <h3 class="text-dark my-3 font-weight-500 text-center">{{current_plan.currency_symbol}}{{current_plan.price}}</h3>
    ...
    <div class="site-wrapper loading-plan" v-else> 
      <div class="px-3 py-2 w-100 plan-header animated-background" style="background: linear-gradient(to right, #eeeeee 8%, lightgray 18%, #eeeeee 33%);">
      </div>
      <div class="plan-container">
...
.plan-card-col .plans-card i{
 ...
 width: 22px;
    ...
    right: -10px;
    position: absolute;
    background-color: #7e249c!important;
    color: #fff!important;
}
.plan-items {
    display: flex;
...
.plans-card{
    ...
    flex: none;
    border:1px solid rgb(130, 48, 198);
    border-radius:6px;
}


.plan-header {
    background-image:url(/all-images/bg1.jpg);
    min-height: 150px;
...
.btn-primary {
   ...
   background:linear-gradient(45deg, #8230c6, #7d208e);
  border:none;
}
.btn-primary:hover {
