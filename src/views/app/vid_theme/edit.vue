  ...
  <div>
    ...
    overflow: hidden;">
      <div class="col-lg-4 border-right " style="height:100vh;overflow:auto;padding:25px;">
        ...
        <div class="card  w-100 mb-2" v-if="current_page.slug=='/' || current_page.slug=='/watching'">
          <button type="button" class="btn btn-danger py-2" @click="add_new=false" v-if="add_new">Cancel Video Block</button>
          <button type="button" class="btn btn-t1 py-2" @click="current_block={name:'Heading', movie_type:'Category', status:true, input_type:'text', specs:[], languages:[], categories:[], movies:[]};getCategories();add_new=true" v-else><i class="fa fa-pencil mr-1" style="font-size:11px;color:white;"></i>Add Video Block</button>
        </div>
      ...
      <div class="col-lg-8 px-0 ">
        <div class="mt-0" style="position:relative" v-if="themename=='vid_theme'">
...
<script>
import SignInPage from '/src/views/app/vid_theme/sign-in'
import HomePage from '/src/views/app/vid_theme/home'
import PlanPage from '/src/views/app/vid_theme/plan'
import VideosPage from '/src/views/app/vid_theme/movies'
import WatchingPage from '/src/views/app/vid_theme/video'
export default {
  components: {
    SignInPage,
    HomePage,
