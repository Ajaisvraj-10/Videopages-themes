    ...
    <div class="site-wrapper2 video-wrapper" :class="menu_click?'nav-active':''">   
            ...
            <div v-if="current_movie.id && start">
                ...
                <div class="flex-align plan-banner" v-if="!frontent_settings.have_plan">
                    <h6 class="text-12" style="line-height:20px;">Ad-Free with Premium now at 30%OFF:$699/year</h6> 
                    <router-link :to="edit?'':{name:'plan'}">
                        <button class="btn" style="background-color: #3b8bef;color: white;font-size: 13px;font-weight: 500;border: 1px solid #3b8bef;border-radius: 5px;padding:8px 10px;width: max-content;">
                        <img src="/video_player_mobile/images/king.png" style="width:16px;" class="mr-1">BUY PLAN</button>
                    </router-link>
                        
                </div>
                <div class="video-details">
                    <h5 class="text-14 font-weight-500 mb-2">{{current_movie.name}} ({{current_movie.language_detail.name}})</h5>
                    <h6 class="text-10">
...
<script>
import MainHeader from '/src/views/app/video_player_mobile/header'
export default {
